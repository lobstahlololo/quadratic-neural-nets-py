...
inline void xavier_initialisation(float* weights, int total_size, const std::vector<LayerArgs>& layers) {
	...
	for (size_t layer_idx = 0; layer_idx < layers.size(); ++layer_idx) {
		...
		if (layer_idx == 0) {
			if (current.kind == Quadratic) {
				// Plain input layer: it has no trainable weights to initialise.
				continue;
			}
			// First parametric layer (embedding): one token id per row.
			input_dimension = 1;
			output_dimension = current.layer_size;
		} else {
			const LayerArgs& previous = layers[layer_idx - 1];
			input_dimension = previous.layer_size;
			output_dimension = current.layer_size;
		}

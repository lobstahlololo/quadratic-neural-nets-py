...
inline std::vector<int> generate_tokens(const std::vector<Layer>& layers, const std::unordered_map<std::string, int>& vocabulary, const std::vector<int>& initial_tokens, int tokens_to_generate, int max_sequence_length) {
	...
	for (int step = 0; step < tokens_to_generate; ++step) {
		while (current_sequence.size() < max_sequence_length) current_sequence.push_back(0);
		if (current_sequence.size() > max_sequence_length) {
			current_sequence.erase(current_sequence.begin(), current_sequence.begin() + (current_sequence.size() - max_sequence_length));
		}
